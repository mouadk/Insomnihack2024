...
import com.applicationsec.rce.ProcessBuilderMethodVisitor;
...
public class RASPClassTransformer implements ClassFileTransformer {
        ...
        public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) {
            if (!List.of("org/springframework/expression/spel/standard/SpelExpressionParser", "java/lang/ProcessBuilder").contains(className)) {
                return null;
            }
            ...
            if(className.equals("java/lang/ProcessBuilder")){
                reader.accept(new ProcessBuilderMethodVisitor(writer), ClassReader.EXPAND_FRAMES);
            } else {
